...
| next | **Network Intelligence milestone** (own architectural milestone per A8: protocol decision gNMI/NETCONF at start, NetworkDevice model, DeviceProtocol impl, switch/router simulator, full Observe→Reason→Act→Verify) — or trained predictive models once outcome data accumulates |
