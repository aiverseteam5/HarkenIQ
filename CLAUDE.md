...
| next | **R5 — remaining candidates:** gNMI/NETCONF network device monitoring (OQ-16 remainder), Console→CC→SM marketplace install automation (transport now exists; wiring is API plumbing), multi-replica audit-chain advisory locking, trained predictive models, CC multi-tenant scoping for patterns/CVE/warranty |
